...
- 🌐 **国家名称标准化** - 按 ISO 两位代码将 GeoLite2 国家名称转换为配置中的标准中文名
- 🇺🇸 **美国州名标准化** - 按外置映射将 GeoLite2 英文州名转换为标准中文名
...
美国 IP 先读取 GeoLite2 的英文州名，再通过 `data/us_states.txt` 映射为标准中文名。
配置格式为每行 `中文名,英文名`；未配置的英文名称会保留原值，避免地理信息丢失。

### 国家名称映射

GeoLite2 国家名称通过 `country.iso_code` 和 `data/countries.txt` 映射为标准中文名。
配置格式为每行 `ISO两位代码,中文名`；未配置的代码会回退到 GeoLite2 提供的名称。
...
生成 `data/ip2region_v4.xdb` 后，可使用本地 maker 镜像运行真实查询测试：

```bash
PYTHONPATH=src .venv/bin/python -m unittest discover \
  -s tests -p 'test_xdb_output.py' -v
```

测试会查询中国、美国、香港、澳门和土耳其的 IPv4，并校验标准化后的国家名称；
美国样本还会校验州名。
